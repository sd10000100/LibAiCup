...
template<typename T>
vector<T> SimulatedAnnealing<T>::getBestWay(){
    return bestPoints_;
}
...
template<typename T>
void SimulatedAnnealing<T>::init(int countItems, vector<T> startPoints,bool writeLog)
{
...
template<typename T>
void SimulatedAnnealing<T>::setPoints(int countItems, vector<T> startPoints)
{
    countItems_= countItems;
    tempPoints_ = startPoints;
    startPoints_ = startPoints;
...
//TODO: переписать на интерфейсный
template<typename T>
...
template<typename T>
void SimulatedAnnealing<T>::findOptimalSequence(){
    time_ = 0;
    unsigned int start_time =  clock();
